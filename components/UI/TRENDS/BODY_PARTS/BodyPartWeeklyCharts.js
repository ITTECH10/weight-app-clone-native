...
const BodyPartCharts = ({ title, dataKey }) => {
    const theme = useTheme()
    const { weeklyBodyPartRecords } = useAppContext()

    // SLICE ON CLIENT IS A CURRENT WORKAROUND! DO ON BACKEND LATER!
    const data = weeklyBodyPartRecords.slice(-7).map((record, i) => {
        return {
            x: i * 2.5,
            y: +record[dataKey]
        }
