...
const BodyPartMonthlyCharts = ({ title, dataKey }) => {
    const { monthlyAverageBodyPartDimensions } = useAppContext()
    const theme = useTheme()

    const data = monthlyAverageBodyPartDimensions.map((record, i) => {
        return {
            x: i * 2.5,
            y: record[dataKey]
        }
    ...
    return (
        <Layout>
            <Text category="h5" style={{ margin: 10 }}>
                Durchschnittliches {title} der letzten 5 Monate
            </Text>
            <MonthsSwitcher />
            <Layout>
                <Chart
...
export default BodyPartMonthlyCharts
